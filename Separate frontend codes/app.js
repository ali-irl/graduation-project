...
const meetings = [
  { id:1, title:'Weekly Progress Check',  date:'2025-12-08', time:'10:00 AM', link:'https://meet.google.com/abc-defg-hij', group:'Team Alpha' },
  { id:2, title:'Midpoint Review',        date:'2025-12-15', time:'11:30 AM', link:'https://meet.google.com/xyz-uvwx-yz1', group:'Team Alpha' },
  { id:3, title:'Final Presentation Prep',date:'2025-12-22', time:'02:00 PM', link:'https://meet.google.com/def-ghij-klm', group:'Team Alpha' },
];

const myProgress = { percent: 68, phase:'System Design', milestones:[
...
// فحص الجلسة عند تحميل الصفحة (تجنب الخروج عند التحديث)
window.addEventListener('DOMContentLoaded', () => {
  const savedRole = sessionStorage.getItem('currentUserRole');

  if (savedRole) {
    // إذا لقينا جلسة محفوظة، بنخلي المتغير الحالي يساويها
    currentRole = savedRole;

    // بنخفي شاشة اختيار الدخول (Home Page) وبنظهر شاشة النظام
    document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
    
    // التأكد إن الشاشة المطلوبة موجودة قبل إظهارها
    const pageElement = document.getElementById('page-' + currentRole);
    if (pageElement) {
        pageElement.classList.add('active');
    }

    // تشغيل الداش بورد المناسبة
...
const API_BASE_URL = 'https://apex.oracle.com/pls/apex/your_workspace/gp_api/'; 
...
function doLogin() {
  // 1. قراءة القيم اللي أدخلها المستخدم من الواجهة
  ...
  if (isNaN(userId)) {
    showToast('danger', 'Invalid ID', 'University ID must contain numbers only.');
    return; 
  }

  // ============== التعديل الجديد هنا ==============
  // 3. حفظ الجلسة في ذاكرة المتصفح
  sessionStorage.setItem('currentUserRole', currentRole);
  // ==============================================

  // 4. الدخول وتغيير الشاشات
  closeLogin();
  document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
            ...
            <div class="d-flex flex-column gap-3">
              ${data.meets.map(m=>`
                <div class="d-flex align-items-center gap-3 p-2 rounded-3" style="background:var(--bg)">
                  ...
                  <a href="${m.link}" target="_blank" class="ms-auto btn-sm-icon text-decoration-none"><i class="bi bi-camera-video"></i></a>
                </div>`).join('')}
            </div>
          </div>
          <div class="card-sgpms p-4">
...
// 2. الدالة الثانية: جلب البيانات (الدكاترة + الطلبات) ورسم الشاشة
async function loadRegistrationData() {
  try {
    // السحر هون: بعثنا كائن (Object) بيحتوي على المصفوفتين مع بعض!
    const data = await fetchMockData({
      sups: supervisors,
      reqs: requests
    });

    // بناء كود الـ HTML واستخدام data.sups و data.reqs
    const contentHtml = `
      <div class="row g-4">
        <div class="col-lg-8">
          <div class="card-sgpms p-4">
            ...
            <div class="row g-3">
              ...
              <div class="col-md-6">
                ...
                <select id="reg-supervisor" class="form-select">
                  ...
                  ${data.sups.map(s=>`<option>${s.name} (${s.area})</option>`).join('')}
                </select>
              </div>
              <div class="col-md-6">
...
async function loadMeetingsData() {
  try {
    // جلب البيانات من مصفوفة meetings
    const data = await fetchMockData(meetings);

    // بناء كود الـ HTML (نفس كودك الأصلي 100% بس استخدمنا متغير data بدل meetings)
    const contentHtml = `
      <div class="row g-4">
        <div class="col-lg-8">
          <div class="d-flex flex-column gap-3">
            ${data.map(m=>`
              <div class="meeting-card">
                ...
                <a href="${m.link}" target="_blank" class="btn-primary-sgpms text-decoration-none text-white" style="border-radius:10px;padding:10px 18px;font-size:0.85rem">
                  <i class="bi bi-camera-video me-2"></i>Join
                </a>
              </div>`).join('')}
          </div>
        </div>
        <div class="col-lg-4">
          <div class="card-sgpms p-4">
            <div class="fw-bold mb-4"><i class="bi bi-calendar3 me-2" style="color:var(--accent)"></i>Meeting Log</div>
            ${[
              {title:'Initial Kick-off',date:'Oct 26',dur:'1hr',notes:'Project scope defined'},
              {title:'Requirement Analysis',date:'Nov 4',dur:'1hr',notes:'Use case diagram reviewed'},
              {title:'Literature Review',date:'Nov 18',dur:'1hr',notes:'3 articles approved'},
              {title:'Technical Specification',date:'Nov 23',dur:'1hr',notes:'Tech stack finalized'},
              {title:'System Modeling',date:'Dec 9',dur:'1hr',notes:'ER diagram submitted'},
            ].map(l=>`
              <div class="d-flex gap-3 mb-3">
                <i class="bi bi-check-circle-fill mt-1" style="color:var(--success);flex-shrink:0"></i>
                <div>
                  <div class="fw-semibold" style="font-size:0.88rem">${l.title}</div>
                  <div class="text-xs text-muted">${l.date} · ${l.dur} · ${l.notes}</div>
                </div>
              </div>`).join('')}
          </div>
        </div>
    ...
    const container = document.getElementById('meetings-container');
    if (container) {
      container.innerHTML = '<div class="alert alert-danger w-100">Failed to load meeting schedule.</div>';
    }
  }
}
...
// 2. الدالة الثانية: جلب بيانات المجموعات ورسم البطاقات (Cards)
async function loadDoctorGroupsData() {
  try {
    // جلب مصفوفة المجموعات باستخدام دالة المحاكاة
    const groups = await fetchMockData(doctorGroups);

    const contentHtml = groups.map(g => `
      <div class="card-sgpms p-4 mb-4">
        <div class="d-flex justify-content-between align-items-start mb-3">
          <div>
            <h5 class="fw-bold mb-1">${g.name}</h5>
            <div class="text-muted" style="font-size:0.9rem">${g.project}</div>
          </div>
          <span class="badge-status badge-ongoing">${g.status}</span>
        </div>
        <div class="row g-3">
          <div class="col-md-6">
            <div class="fw-semibold small mb-2">Team Members</div>
            ${g.members.map(m => `
              <div class="d-flex align-items-center gap-2 mb-2">
                <div style="width:30px;height:30px;border-radius:8px;background:var(--primary);color:#fff;display:flex;align-items:center;justify-content:center;font-size:0.7rem;font-weight:700">
          ...
          <div class="col-md-6">
            <div class="fw-semibold small mb-2">Progress: ${g.progress}%</div>
            <div class="progress-sgpms mb-3"><div class="bar" style="width:${g.progress}%"></div></div>
            <div class="d-flex gap-2">
              <button class="btn-primary-sgpms btn-sm" onclick="showDoctorSection('progress',null)">Update Progress</button>
              <button class="btn-accent btn-sm" onclick="showDoctorSection('meetings',null)">Schedule Meeting</button>
            </div>
          </div>
        </div>
      </div>`).join('');

    // استبدال دائرة التحميل بقائمة المجموعات الحقيقية
    const container = document.getElementById('groups-list-container');
    if (container) {
      container.className = ''; // إزالة كلاس السبنر لتعديل التصميم
      container.innerHTML = contentHtml;
    }
  } catch (error) {
    console.error("Error loading doctor groups:", error);
    const container = document.getElementById('groups-list-container');
    if (container) {
      container.innerHTML = '<div class="alert alert-danger w-100">Failed to load groups. Please try again.</div>';
    }
  }
}
...
function doctorMeetings() {
  loadDocMeetingsData(); // جلب البيانات بالخلفية

  return `
  <div class="page-header" style="background:linear-gradient(120deg,#0f2a25,#1a3530)">
    <div class="page-title">Schedule Meetings</div>
    <div class="page-sub mt-1">Create and manage group meeting schedules</div>
  </div>
  
  <div id="doc-meetings-container" class="spinner-container">
    <div class="spinner"></div>
  </div>`;
}

// 2. الدالة الثانية: جلب البيانات ورسم واجهة المواعيد
async function loadDocMeetingsData() {
  try {
    // جلب بيانات المجموعات والاجتماعات معاً
    const data = await fetchMockData({
      groups: doctorGroups,
      meets: meetings
    });
    ...
    const contentHtml = `
      <div class="row g-4">
        <div class="col-lg-6">
          <div class="card-sgpms p-4">
            <div class="fw-bold mb-4"><i class="bi bi-calendar-plus me-2" style="color:var(--doc-accent)"></i>New Meeting</div>
            <div class="row g-3">
              <div class="col-12"><label class="form-label">Select Group</label>
                <select class="form-select">
                  ${data.groups.map(g=>`<option>${g.name} – ${g.project}</option>`).join('')}
                </select>
              </div>
              <div class="col-12"><label class="form-label">Meeting Title</label><input class="form-control" placeholder="e.g. Weekly Progress Check"/></div>
              <div class="col-md-6"><label class="form-label">Date</label><input type="date" class="form-control"/></div>
              <div class="col-md-6"><label class="form-label">Time</label><input type="time" class="form-control"/></div>
              <div class="col-12"><label class="form-label">Meeting Link / Location</label><input class="form-control" placeholder="Zoom/Google Meet link or room number"/></div>
              <div class="col-12"><label class="form-label">Notes for Students</label><textarea class="form-control" rows="3" placeholder="Topics to prepare…"></textarea></div>
              <div class="col-12"><button class="btn-primary-sgpms" onclick="showToast('success','Meeting Scheduled','Students have been notified.')"><i class="bi bi-send me-2"></i>Send Schedule</button></div>
            </div>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="card-sgpms p-4">
            ...
            <div class="d-flex flex-column gap-3">
              ${data.meets.map(m=>`
                <div class="meeting-card" style="border-left-color:var(--doc-accent)">
                  <div style="width:50px;height:50px;border-radius:12px;background:linear-gradient(135deg,var(--doc-accent),#45a895);color:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;font-size:0.78rem;font-weight:700;flex-shrink:0">
                    <span>${new Date(m.date).toLocaleString('en',{month:'short'})}</span>
                    <span style="font-size:1.1rem">${m.date.slice(8)}</span>
                  </div>
                  <div class="flex-1">
                    <div class="meeting-title">${m.title}</div>
                    <div class="meeting-time">${m.group} · ${m.time}</div>
                  </div>
                  <button class="btn-sm-icon" title="Edit" onclick="showToast('info','Edit Meeting','Edit form will open.')"><i class="bi bi-pencil"></i></button>
                </div>`).join('')}
            </div>
          </div>
        </div>
    ...
    const container = document.getElementById('doc-meetings-container');
    if (container) {
      container.className = ''; 
      container.innerHTML = contentHtml;
    }
  } catch (error) {
    console.error("Error loading doctor meetings:", error);
    const container = document.getElementById('doc-meetings-container');
    if (container) {
      container.innerHTML = '<div class="alert alert-danger w-100">Failed to load meeting data.</div>';
    }
  }
}
...
/* ─── AI Chat ─── */
// 1. الدالة الأولى: أنيميشن تحميل التبويب نفسه أول ما تكبس عليه
function studentAI() {
  loadAIData(); // بتبلش تجيب الداتا بالخلفية
  ...
  <div class="page-header" style="background:linear-gradient(120deg,#0f2540,#1a3a5c)">
    ...
    <div class="page-sub mt-1">Powered by Claude · Get personalized graduation project ideas</div>
  </div>
  
  <!-- هاي دائرة التحميل للتبويب كامل -->
  <div id="ai-container" class="spinner-container">
    <div class="spinner"></div>
  </div>`;
}

// 2. الدالة الثانية: رسم شاشة الذكاء الاصطناعي بعد ما يخلص التحميل
async function loadAIData() {
  try {
    // جلب قائمة المشاريع السابقة من المصفوفة اللي فوق
    const data = await fetchMockData(aiProjects);
      ...
      <div class="row g-4">
        <div class="col-lg-8">
          <div class="ai-chat-wrap">
            ...
            <div class="ai-chat-msgs" id="ai-msgs">
              <div class="msg bot">
                <div class="msg-bubble">👋 Hello! I'm your AI Project Assistant.<br><br>I can help you <strong>generate graduation project ideas</strong> based on your major, skills, and interests. Just tell me about yourself or choose a domain below!</div>
                <div class="msg-time">Now</div>
              </div>
              <div class="d-flex flex-wrap gap-2 mt-1">
                ${['AI & Machine Learning','Web Development','Cybersecurity','Data Science','Mobile Development'].map(d=>`
                  <button class="chip" style="cursor:pointer;border:none;background:rgba(26,58,92,0.08)" onclick="sendAIMsg('${d}')">
                    ${d}
                  </button>`).join('')}
              </div>
            </div>
            <div class="ai-chat-input">
              <input id="ai-input" placeholder="Type your message… e.g. I love AI and Python" onkeydown="if(event.key==='Enter')sendAIChat()"/>
        ...
        <div class="col-lg-4">
          <div class="card-sgpms p-4 mb-4">
            ...
            ${[['1','Tell me your major & skills','e.g. CIS, Python, React'],['2','Choose a domain','AI, Web, Security, Data…'],['3','Get tailored ideas','Titles, descriptions & references']].map(([n,t,s])=>`
              <div class="d-flex gap-3 mb-3">
                <div style="width:28px;height:28px;border-radius:8px;background:var(--primary);color:#fff;display:flex;align-items:center;justify-content:center;font-size:0.8rem;font-weight:700;flex-shrink:0">${n}</div>
                <div><div class="fw-semibold" style="font-size:0.88rem">${t}</div><div class="text-xs text-muted">${s}</div></div>
              </div>`).join('')}
          </div>
          <div class="card-sgpms p-4">
            <div class="fw-bold mb-3"><i class="bi bi-archive me-2" style="color:var(--accent)"></i>Previous JUST Projects</div>
            ${data.map(p=>`
              <div class="d-flex align-items-center gap-2 mb-2 p-2 rounded-3" style="background:var(--bg)">
    ...
    const container = document.getElementById('ai-container');
    if (container) container.innerHTML = '<div class="alert alert-danger w-100">Failed to load AI interface.</div>';
  }
}

// 3. دالة الاستقبال من حقل الإدخال
function sendAIChat() {
...
// 4. دالة المحادثة (اللي فيها أنيميشن النقاط اللي بتضوي وتطفي)
async function sendAIMsg(msg) {
  ...
  let key = Object.keys(aiSuggestMap).find(k => lm.includes(k)) || Object.keys(aiSuggestMap)[Math.floor(Math.random()*Object.keys(aiSuggestMap).length)];

  try {
    const ideas = await fetchMockData(aiSuggestMap[key]);
    const typing = document.getElementById(typingId);
    if (!typing) return;

    const response = `Based on your interest in <strong>${msg}</strong>, here are 3 tailored graduation project ideas:<br><br>
      ${ideas.map((idea,i)=>`<strong>${i+1}. ${idea}</strong>`).join('<br>')}
      <br><br>Each of these aligns well with JUST's CIS curriculum. Would you like me to elaborate on any of these? 💡`;
    ...
    const typing = document.getElementById(typingId);
    if (typing) typing.outerHTML = `<div class="msg bot"><div class="msg-bubble text-danger">Error connecting to AI.</div></div>`;
  }
}
